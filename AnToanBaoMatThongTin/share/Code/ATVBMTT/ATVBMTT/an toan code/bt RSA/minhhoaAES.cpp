...
unsigned int SubWord(unsigned int w)
{
	...
	for(int i = 1; i <= 4; i++)	
	{
        unsigned int byte1 = (w>>(32 -i*8))&0xFF;
        unsigned int sub8 = S[byte1];
        kq = (kq<<8) | sub8;
	}
...
{
	unsigned int rotW = RotWord(w); // dich 1 bit
	unsigned int subW = SubWord(rotW); // subword
	unsigned int kq = XorRcon(subW, j); // xor with rc
	return kq;
}
